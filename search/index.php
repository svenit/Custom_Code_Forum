<?php
  ...
  if(isset($_GET['query'])){
      ...
      $newNews = $connect->query("SELECT * FROM table_baiviet WHERE title LIKE '%$query%'");
      $limit = 5;
     ...
     $start = ($page - 1)*$limit;
     $arrpost = $connect->query("SELECT * FROM table_baiviet WHERE title LIKE '%$query%'  ORDER BY id DESC LIMIT $start,$limit ");
     $countQuery = $connect->query("SELECT * FROM table_baiviet WHERE title LIKE '%$query%'  ORDER BY id DESC");
     $numQuery = $countQuery->num_rows;
  }
  if(!trim($_GET['query']) || $_GET['query'] == ""){
  ...
  <div id="body">
    ...
    <div id="col-left">
      <div id="direct">
        ...
        <a href=""><span class="direct-object upper">Tìm kiếm</span></a>
      </div>
      <div id="box-news">
        <?php
            if($numQuery > 0){
             ?> <p class='search-result'>Có <span class='hl-search'><?php echo $numQuery ?></span> kết quả cho <span class='hl-search'><?php echo $query; ?></span></p> <?php
            ...
            <?php
             ...
             ?>
               <div id="box-container">
                  <div id="none-result-search">
                   <p class='search-result'>Không có kết quả nào cho <span class='hl-search'><?php echo $query; ?></span></p>
                   <center><img src='https://cdn.dribbble.com/users/463734/screenshots/2016792/empty-result_shot_1x.png'></center>
                 </div>
               </div>
             <?php
         }
               ...
               <?php 
                     for($i = 1;$i <= $each_page;$i++){
                        if($i > $page + 2  || $i < $page - 2 ){
                           ...
                           ?>
                              <li><a href="?page=<?php echo $i ?>&query=<?php echo $query; ?>" class="<?php if($i == $page){echo 'thispage';} ?>"><?php echo $i ?></a></li>
                           <?php
                         }
                      }
                ?>
